...
void rectMissile::render()
{
	for (_viBullet = _vBullet.begin(); _viBullet != _vBullet.end(); ++_viBullet)
	{
		...
		_viBullet->bulletImage->alphaFrameRender(getMemDC(),
			_viBullet->rc.left,
			_viBullet->rc.top,
			_viBullet->bulletImage->getFrameX(), 0, _viBullet->alphaValue);
...
void rectMissile::move()
{
	for (_viBullet = _vBullet.begin(); _viBullet != _vBullet.end();)
	{
		if (_viBullet->alphaValue > 0) _viBullet->alphaValue-= 5; // 그냥 --하니까 너무 천천히 사라져서..
		else _viBullet->alphaValue = 255;
		_viBullet->y -= _viBullet->speed;
		_viBullet->rc = RectMakeCenter(_viBullet->x, _viBullet->y,
