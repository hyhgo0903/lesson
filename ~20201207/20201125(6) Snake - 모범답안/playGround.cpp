...
HRESULT playGround::init()
{
	...
	for (int i = 0; i < SNAKEBODY; i++)
	{
		...
		_snake[i].radius = 20;
		_snake[i].x = (i == 0) ? RND->getFromFloatTo(100, WINSIZEX - 100) : _snake[i - 1].x - cosf(_snake[i].angle) * (_snake[i].radius * 2);
		_snake[i].y = (i == 0) ? RND->getFromFloatTo(100, WINSIZEY - 100) : _snake[i - 1].y - (-sinf(_snake[i].angle) * (_snake[i].radius * 2));
...
void playGround::update()
{
	...
	//배열 0번째 인덱스가 머리겠죠
	if (KEYMANAGER->isStayKeyDown(VK_LEFT)) _snake[0].angle += 0.1f;
	if (KEYMANAGER->isStayKeyDown(VK_RIGHT)) _snake[0].angle -= 0.1f;
...
void playGround::render(HDC hdc)
{
	...
	{
		HBRUSH brush = CreateSolidBrush(RGB(255,255- i * 5,255- i * 5));
