...
void playGround::update()
{
	...
	if (!_isDead && _isStart)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			_ground[i].left -= 5; _ground[i].right -= 5;
			_obstacle[i].left -= 5; _obstacle[i].right -= 5;
		...
		for (int i = 0; i < 3; i++)
		{
			if (IntersectRect(&temp, &_rc, &_ground[i]))
			{
				...
				else if (_rc.bottom > _ground[i].top	&& _rc.top < _ground[i].top)
				{// 아래로 부딪힐때 착지
					if(_jumpPower <0) _jumpPower = 0;
					_gravity = 0.0f;
					_y = WINSIZEY / 2;
					// _y말고 top bottom건드렸을때 작동이상함 왤까
					status = GROUND;
				}
			...
			if (IntersectRect(&temp, &_rc, &_obstacle[i]))
			{
				...
				else if (_rc.bottom > _obstacle[i].top	&& _rc.top < _obstacle[i].top)
				{// 아래로 부딪힐때 착지
					if (_jumpPower < 0) _jumpPower = 0;
					_gravity = 0.0f;
					_jumpCount = 2;
					_y = _obstacle[i].top - 40;
					// _y말고 top bottom건드렸을때 작동이상함
					status = GROUND;
				}
		...
		_rc2 = RectMakeCenter(_x, _y + 1, 80, 80);
		if (status == GROUND &&
...
void playGround::render(HDC hdc)
{
	...
	sprintf_s(str, "현재 점수 : %d", score);
	TextOut(backDC, WINSIZEX / 2 - 50, 50, str, strlen(str));
	sprintf_s(str, "최고 점수 : %d", highScore);
