...
public:
	...
	{
		return _imageInfo->maxFrameX <= 0 ? // 이미지가 한장짜리니?
			_imageInfo->x + (_imageInfo->width / 2) : // 전체크기에서 절반 반환
			_imageInfo->x + (_imageInfo->frameWidth / 2); // 프레임 너비에서 절반
	}
	...
	inline void setFrameX(int frameX)
	{
		...
		if (frameX > _imageInfo->maxFrameX) // 만약 0~9번프레임까지 있는데 10번이다? 예외처리
		{ // max = frame 번호에서 -1(인덱스)
