...
void sceneManager::render(POINT camera)
{
	if (_currentScene) _currentScene->render(camera);
}
...
HRESULT sceneManager::changeScene(string sceneName, bool check)
{
	//이터레이터에 찾고자 하는 씬의 키 값을 넣는다
	mapSceneIter find = _mSceneList.find(sceneName);
	...
	//씬을 바꾸는데 성공했으면  init()함수를 바로 실행시킨다
	if (SUCCEEDED(find->second->init(check)))
	{
		//현재 씬의 릴리즈 함수를 실행시켜 메모리 해제를 하고
		if (_currentScene) _currentScene->release();
